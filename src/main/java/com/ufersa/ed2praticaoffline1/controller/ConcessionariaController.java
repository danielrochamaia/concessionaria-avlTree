package com.ufersa.ed2praticaoffline1.controller;
...
public class ConcessionariaController {
    ...
    public ResponseEntity<AutomovelResponse> BuscarAutomovelPorChave(@PathVariable int chave) {
        var response = concessionariaClient.buscarAutomovel(chave);
        return new ResponseEntity<>(response, HttpStatus.OK);
    }

    @GetMapping("/automovel/{renavam}/{placa}")
    public ResponseEntity<AutomovelResponse> BuscarAutomovelRenavamPlaca(@PathVariable String renavam, @PathVariable String placa) {
        var response = concessionariaClient.buscarAutomovelRenavamPlaca(renavam, placa);
        return new ResponseEntity<>(response, HttpStatus.OK);
    }

    @DeleteMapping("/automovel/{chave}")
    public ResponseEntity<String> RemoverAutomovel(@PathVariable int chave) {
        var response = concessionariaClient.deletarAutomovel(chave);
        return new ResponseEntity<>(response, HttpStatus.OK);
    }

    @PutMapping("/automovel/{chave}")
    public ResponseEntity<String> EditarAutomovel(@RequestBody AutomovelProtocolo automovelProtocolo, @PathVariable int chave){
